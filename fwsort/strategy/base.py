...
所有信号源（随机、外部HTTP、Webhook等）都必须实现 StrategyBase 接口。
统一信号格式：
...
class StrategyBase(ABC):
    ...
    def should_open(self, signal: Signal, ctx: dict) -> tuple[bool, str]:
        ...
        在信号有效后、下单前调用。子类可重写以实现自定义开仓条件。
        默认放行。

        Args:
            signal: 信号对象
            ctx: 上下文，包含 task / now / gateway 等

        Returns:
            (allow, reason): allow=True 表示允许开仓，reason 为说明
        """
        return True, "default pass"

    def execute_order(self, signal: Signal, gateway, ctx: dict) -> dict | None:
        """下单执行钩子（可选覆盖）

        子类可重写以自定义下单逻辑（如分批、限价等）。
        返回 None 表示走 service 默认下单流程（_execute_order_with_retry）。
        返回 dict 表示自定义下单结果。
        ...
        Returns:
            dict | None: None=走默认流程，dict=自定义下单结果
        """
        return None
