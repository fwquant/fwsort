...
def _execute_task_in_thread(task_id: int) -> None:
    ...
    try:
        ...
        logger.debug(f"[AutoStrategyDispatcher] task={task_id} released from executing set")


def _dispatcher_loop() -> None:
    """调度器主循环：定期检查并触发到期任务"""
    global _last_heartbeat_time
    logger.info("[AutoStrategyDispatcher] ✅ 内置自动任务调度器已启动")
    ...
    logger.info("[AutoStrategyDispatcher] ⛔ 内置自动任务调度器已停止")
...
def _scan_and_dispatch() -> None:
    ...
    if current_time - _last_heartbeat_time >= HEARTBEAT_INTERVAL:
        ...
        if task_count > 0 or executing_count > 0:
            logger.info(
                f"[AutoStrategyDispatcher] 💓 心跳: {task_count} 活跃任务, "
...
def _process_single_task(task: AutoStrategy, now: int) -> None:
    ...
    with _executing_lock:
        if task_id in _executing_tasks:
            logger.debug(f"[AutoStrategyDispatcher] 跳过正在执行的任务 task={task_id}")
            return
    ...
    if last_run == 0 or elapsed >= interval_seconds:
        ...
        logger.info(
            f"[AutoStrategyDispatcher] 🎯 触发任务 task={task_id} name={task.task_name} "
        ...
        with _executing_lock:
            if len(_executing_tasks) >= MAX_CONCURRENT_EXECUTIONS:
                logger.warning(
                    f"[AutoStrategyDispatcher] 最大并发执行数({MAX_CONCURRENT_EXECUTIONS})已达，跳过 task={task_id}"
                )
                return
        ...
        thread.start()
        logger.info(f"[AutoStrategyDispatcher] 🚀 任务 task={task_id} 已提交到执行线程")
    else:
        # 还未到执行时间
        remaining = interval_seconds - elapsed
        logger.debug(
            f"[AutoStrategyDispatcher] ⏳ 任务 task={task_id} 未到期，还需 {remaining}s "
...
def start_dispatcher() -> None:
    ...
    if _dispatcher_thread and _dispatcher_thread.is_alive():
        logger.warning("[AutoStrategyDispatcher] dispatcher already running")
        return
    ...
    logger.info("[AutoStrategyDispatcher] dispatcher thread started")
...
def stop_dispatcher() -> None:
    ...
    if _dispatcher_thread and _dispatcher_thread.is_alive():
        ...
        logger.info("[AutoStrategyDispatcher] dispatcher thread stopped")
