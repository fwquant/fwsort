"""信号源文件管理器 - 基于 .py 文件的信号源 CRUD

职责：
    - 扫描 providers/ 目录，获取所有信号源
    - 读取单个信号源的参数元信息
    - 更新信号源的参数值（直接修改 .py 文件中的类属性赋值行）
    - 创建新信号源（生成 .py 文件）
    - 删除信号源（删除 .py 文件）
...
from fwsort.signals.base import (
    PARAM_TYPE_NAMES,
...
def _build_provider_info(name: str, cls: type[SignalProvider]) -> dict[str, Any]:
    """构建单个信号源的元数据"""
...
def update_provider_values(provider_name: str, values: dict[str, Any]) -> dict[str, Any]:
    ...
    for param_name, new_value in values.items():
        ...
        try:
            coerced = SignalProvider.coerce_param_value(new_value, target_type)
            is_valid, err = SignalProvider.validate_param_value(coerced)
            if not is_valid:
...
def create_provider(
    ...
    module_path = f"fwsort.signals.providers.{file_stem}"
...
def _build_http_provider_code(
    ...
    return f'''"""HTTP URL 信号源: {provider_name}
