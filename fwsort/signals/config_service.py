...
本模块保留用于向后兼容，实际逻辑已迁移到 fwsort.signals.file_manager。
所有函数都是 file_manager 对应函数的薄封装。

新架构：
    - 信号源配置存储在 providers/ 目录下的 .py 文件类属性默认值中
    - 参数声明通过 parameters 列表在 SignalProvider 子类中定义
    - Web 界面编辑参数时直接修改 .py 文件并触发热加载
"""
from __future__ import annotations

from fwsort.signals.file_manager import (
...
def sync_builtin_providers() -> int:
    """已废弃：新架构自动发现 providers/ 目录下所有信号源"""
    return 0
